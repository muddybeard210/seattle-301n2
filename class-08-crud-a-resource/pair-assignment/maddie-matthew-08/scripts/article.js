(function(module) {
  ...
  Article.createTable = function(callback) {
    webDB.execute(
      'CREATE TABLE IF NOT EXISTS articles (id INTEGER PRIMARY KEY, title VARCHAR(255) NOT NULL, category VARCHAR(255), author VARCHAR(255) NOT NULL, authorUrl VARCHAR(255), publishedOn VARCHAR(255), body TEXT NOT NULL)',
    ...
    webDB.execute('SELECT * FROM articles', function(rows) {
      if (rows.length) {
        ...
        $.getJSON('/data/hackerIpsum.json', function(rawData) {
          localStorage.setItem('rawData', JSON.stringify(rawData));// Cache the json, so we don't need to request it next time:
